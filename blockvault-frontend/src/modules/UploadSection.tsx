      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary/50 text-xs">🔒</span>
            <Input label="Passphrase" value={key} onChange={e => setKey(e.target.value)} placeholder="encryption key" revealToggle className="pl-8" />
          </div>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary/50 text-xs">ℹ️</span>
            <Input label="AAD (optional)" value={aad} onChange={e => setAad(e.target.value)} placeholder="associated data" className="pl-8" />
          </div>
        </div>
        <div className="flex items-center gap-4">
          <Button onClick={upload} disabled={!file} variant="primary" loading={progress>0 && progress<100} leftIcon={<span className="text-lg -ml-1">🛡️</span>}>Encrypt & Upload</Button>
