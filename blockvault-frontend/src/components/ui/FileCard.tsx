...
export const FileCard: React.FC<FileCardProps> = ({ f, onDownload, onDelete, onVerify, onShare, onCopyCid, onCopyHash, onPreview, compact }) => {
    ...
    <div className="group relative p-4 rounded-xl border border-border/40 bg-background-tertiary/30 backdrop-blur-sm hover:border-accent-blue/60 hover:shadow-[0_0_0_1px_#00C0FF] transition grid grid-cols-[auto_1fr_auto] gap-4 items-start">
      ...
      <div className="flex flex-col items-end gap-2">
        <div className="flex gap-1">
          {onShare && (
            <Button size="sm" variant="secondary" onClick={onShare} title="Share" leftIcon={<Icon name="share" size={14} className="text-accent-blue" />}>Share</Button>
          )}
          {onPreview && (
            <Button size="sm" variant="outline" onClick={onPreview} title="Preview" leftIcon={<Icon name="eye" size={14} className="text-accent-blue" />}>Preview</Button>
          )}
          <Button size="sm" variant="outline" onClick={onDownload} title="Download" leftIcon={<Icon name="download" size={14} className="text-accent-blue" />}>Download</Button>
          <Button size="sm" variant="danger" onClick={onDelete} title="Delete" leftIcon={<Icon name="delete" size={14} className="text-accent-red" />}>Delete</Button>
          <Button size="sm" variant="secondary" onClick={onVerify} title="Verify" leftIcon={<Icon name="verify" size={14} className="text-accent-green" />}>Verify</Button>
          <button
